...
function validate(e) {
  e.preventDefault();
  var _validUserName = '[A-Za-z]{8}';
  var _validEmail = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
  var error_name = document.getElementById('error_name');
  var error_email = document.getElementById('error_email');
  var avatar = document.querySelector('.fa-user');
  var envelope = document.getElementById('envelope');
  var border_red = '2px solid red';
  var border_green = '2px solid green';
  ...
  if (document.getElementById('input_name').value.match(_validUserName) && document.getElementById('input_email').value.match(_validEmail)) {
    sendEmail(); //   window.location.href = 'mailTo:mirelmandrut94@yahoo.com'
  }
...
function show_icons() {
  var container_icons = document.querySelector('.container_card_icons');
...
show_icons(); //END ICONS CONTAINER CODE
//DARK MODE CODE
...
function sendEmail() {
  ...
  var from = document.getElementById('input_email').value;
  ...
  emailjs.send('gmail', 'template_dTtINCkM', templateParams).then(function (response) {
